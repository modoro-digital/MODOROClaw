...
1. **tools.allow hardened** — global allowlist reduced to 4 safe tools: `message`, `web_search`, `web_fetch`, `update_plan`. Dangerous tools (`exec`, `process`, `cron`, `read_file`, `write_file`, `list_files`, `search_files`) removed. CEO operations go through `web_fetch` to authenticated local API on port 20200 (`/api/file/*`, `/api/exec`, `/api/cron/*`, `/api/zalo/send`). COMMAND-BLOCK (layer 2) + AGENTS.md (layer 3) provide defense-in-depth.
...
**One-way cron (CEO Telegram → Zalo groups):** Local HTTP Cron API on port 20200 (`startCronApi()`). Bot uses `web_fetch` tool (in tools.allow) to call `http://127.0.0.1:20200/api/cron/*`. API validates groupId against groups.json, validates cronExpr, writes custom-crons.json, restarts cron scheduler. **5-layer protection against Zalo abuse:** (a) command-block rewrites rawBody for admin patterns AND API URL mentions, (b) cron/exec tools removed from tools.allow, (c) rotating auth token (48 hex chars, regenerated every boot) injected into AGENTS.md at boot — NOT readable from any unauthenticated endpoint, (d) ALL dangerous endpoints (file/read, file/write, exec, system/info) require token — only workspace/read (strict whitelist) and cron/list are public, (e) API binds localhost only. Write operations serialized via async mutex.
