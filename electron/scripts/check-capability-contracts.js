...
for (const rel of files) {
  ...
  const highRisk = sideEffects.some(s => /may-send-zalo|creates-cron/i.test(s));
...
const imageSkill = readText('skills/operations/image-generation.md');
if (/workspace\/read\?path=cron-api-token\.txt/i.test(imageSkill)) {
