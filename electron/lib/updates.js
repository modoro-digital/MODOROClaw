...
const MAX_RELEASE_BODY_SIZE = 1024 * 1024; // 1MB cap on GitHub API response
let _latestRelease = null; // cached { version, body, html_url, assets } — no TTL, invalidated by checkForUpdates
  ...
  return new Promise((resolve) => {
    ...
    const req = https.get(opts, (res) => {
      ...
      res.on('data', (chunk) => {
        ...
        // M3: cap response body to prevent memory abuse
        if (dataLen > MAX_RELEASE_BODY_SIZE) { req.destroy(); return resolve(null); }
        data += chunk;
      });
      res.on('end', () => {
        try {
