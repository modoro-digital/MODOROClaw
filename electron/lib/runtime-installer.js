...
const SHARED_VERSIONS = (() => {
  try {
    ...
    return { openclaw: '2026.4.14', openzca: '0.1.59', nineRouter: '0.4.63', node: '22.22.2' };
  }
})();
...
const MAC_GIT_SHIM = [
  ...
  '# npm prepends flags before the subcommand — strip them:',
  '#   npm 10.x: git --no-replace-objects clone ...',
  '#   npm 6.x:  git -c transfer.fsckobjects=false clone ...',
  'while true; do',
  '  case "$1" in',
  '    -c) shift; shift;;',
  '    --no-replace-objects|--bare|--literal-pathspecs) shift;;',
  '    --git-dir=*|--work-tree=*) shift;;',
  '    --git-dir|--work-tree) shift; shift;;',
  '    *) break;;',
  '  esac',
  'done',
  'case "$1" in',
...
async function runInstallation({ onProgress } = {}) {
  ...
  try {
    ...
    // Step 4: Download embedding model (non-fatal — grep fallback exists)
