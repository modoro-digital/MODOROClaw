...
function getBundledVendorDir() {
  ...
  } catch { /* fs access fail is expected on first launch — vendor not yet downloaded */ }
...
function getBundledNodeBin() {
  ...
  } catch { /* expected: userData not available in dev mode or before app.whenReady */ }
...
function enumerateNodeManagerBinDirs() {
  ...
  // Nested loops: root dirs → version dirs → bin dirs. Depth is acceptable
  // because the outer try/catch swallows all errors (non-existent dirs are common).
  try {
...
async function findOpenClawBin() {
  ...
  console.warn('[findOpenClawBin] no openclaw binary found in any candidate path');
  return null;
}
...
function findNodeBin() {
  ...
  console.error('[findNodeBin] FAILED — no Node binary found in any candidate location. Install Node.js (https://nodejs.org/) or check PATH.');
  _cachedNodeBin = '';
  return null;
}
...
async function spawnOpenClawSafe(args, { timeoutMs = 600000, cwd, allowCmdShellFallback = true, signal } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code, sig) => { clearTimeout(killer); resolve({ code: code !== null ? code : (sig ? -9 : -1), stdout, stderr, viaCmdShell, aborted: !!(signal && signal.aborted) }); });
  });
}
...
function bootDiagRunFullCheck() {
  ...
  // Surface critical failures to a flag file so Dashboard overview can show alerts
  try {
    const criticalLines = (_bootDiagState?.lines || []).filter(l => l.includes('✗')); // ✗ marker
    if (criticalLines.length > 0) {
      const flagFile = path.join(getWorkspace(), 'logs', 'boot-diagnostic-critical.flag');
      fs.writeFileSync(flagFile, JSON.stringify({ ts: new Date().toISOString(), issues: criticalLines }), 'utf-8');
    }
  } catch {}
}
