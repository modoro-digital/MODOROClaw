...
export async function handleOpenzaloInbound(params: {
  ...
  if (hasMedia && !rawBody) {
    ...
    if (__mtIsNonText) {
      runtime.log?.(`openzalo: skip AI for non-text media (${__mtTypes.join(",")}) from ${message.senderId}`);
      return;
    }
  }
  ...
  if (!message.isGroup && rawBody) {
    ...
    if (__osMatch) {
      runtime.log?.(`openzalo: OUT-OF-SCOPE blocked from ${message.senderId}: ${rawBody.slice(0, 80)}`);
      try {
        ...
        runtime.error?.(`openzalo: OUT-OF-SCOPE auto-reply failed: ${String(__osErr?.message || __osErr).slice(0, 120)}`);
      }
      return;
    }
  }
...
${__ragNeutralize(r.snippet).slice(0, 500)}
  ...
  try {
    ...
    if (__pzIsBotCmd) {
      runtime.log?.("openzalo: drop bot command (pause is Dashboard-only): " + __pzBody);
      return;
    }
  ...
  const __mcDoDeliver = async (payload: any) => {
    ...
    try {
      ...
      if (__drTransient) {
        runtime.log?.(`openzalo: deliver failed (transient: ${__drMsg.slice(0, 120)}) — retrying in 2s`);
        ...
        try {
          ...
          runtime.log?.("openzalo: deliver retry succeeded");
        } catch (__drRetryErr: any) {
          runtime.error?.(`openzalo: deliver retry also failed: ${String(__drRetryErr?.message || __drRetryErr).slice(0, 200)}`);
        }
      } else {
        runtime.error?.(`openzalo: deliver failed (non-transient): ${__drMsg.slice(0, 200)}`);
      }
    }
  };
